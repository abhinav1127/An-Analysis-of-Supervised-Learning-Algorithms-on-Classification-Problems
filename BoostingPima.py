...
for max_depth in max_depths:
    dtClassifier = tree.DecisionTreeClassifier(max_depth=max_depth)
    clf = AdaBoostClassifier(dtClassifier, n_estimators=100)
...
plt.title("Pruning Metrics")
plt.draw()
plt.savefig("MetricsBoostingPD.png")
...
depth = CV_results.index(max(CV_results))
depth += 1

classifier = AdaBoostClassifier(tree.DecisionTreeClassifier(criterion= 'gini', max_depth = depth), n_estimators=100)
...
plot_confusion_matrix(cnf_matrix, classes=[0, 1],
                      title='Confusion Matrix')
plt.savefig("CFMatrix_BoostingPD.png")

plt.figure()
plot_learning_curve(classifier, "Learning Curve", X_train, y_train, cv=3)
plt.savefig("LearningCurveBoostingPD.png")
