...
plt.draw()
plt.savefig("Pre-pruningMetricsPI.png")
...
plt.savefig("CFMatrix_prepruningPI.png")

plt.figure()
plot_learning_curve(classifier, "Learning Curve", X_train, y_train, cv=3)
plt.savefig("LearningCurveDTPI.png")
...
plt.draw()
plt.savefig("Post-PruningPD.png")

threshold = AUC_values.index(max(AUC_values))
print(threshold, 'threshold')
# threshold += 1

numOfNodesRemoved = prune_index(postPrunedClassifier.tree_, 0, threshold, 0)
y_prediction = postPrunedClassifier.predict(X_test)
...
plt.savefig("CFMatrix_postpruningPD.png")
